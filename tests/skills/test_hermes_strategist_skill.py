"""Smoke tests for the hermes-strategist skill script.
...
SCRIPT = REPO_ROOT / "skills" / "devops" / "hermes-strategist" / "scripts" / "strategist.sh"
...
def test_once_handles_missing_hermes(tmp_path: Path) -> None:
    bash = shutil.which('bash')
    if bash is None:
        pytest.skip('bash not on PATH')
    # The script must not crash even if 'hermes' is not on PATH.
    env = os.environ.copy()
    env['PATH'] = str(tmp_path)  # an empty directory
    result = subprocess.run(
