"""Smoke tests for the project-watchdog skill script.
...
SCRIPT = REPO_ROOT / "skills" / "devops" / "project-watchdog" / "scripts" / "watchdog.sh"
...
def test_once_with_no_package_json(tmp_path: Path) -> None:
    bash = shutil.which('bash')
    if bash is None:
        pytest.skip('bash not on PATH')
    result = subprocess.run(
    ...
    # With no package.json, the script logs and exits 0 (graceful no-op).
    assert 'no package.json' in result.stdout or 'skipping' in result.stdout
