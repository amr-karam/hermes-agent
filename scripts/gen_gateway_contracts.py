...
KOTLIN_OUT = ROOT / "apps" / "android" / "app" / "src" / "main" / "java" / "com" / "hermes" / "gateway" / "gateway-contract.kt"
_KOTLIN_HEADER = (
...
class KotlinRenderer:
    """Render JSON Schema defs into Kotlin data classes with kotlinx.serialization."""
    ...
    def kotlin_type(self, schema: dict, *, inline_depth: int = 0) -> str:
        """Return a Kotlin type string from a JSON Schema dict."""
        if "$ref" in schema:
            name = self.ref_name(schema["$ref"])
            self.ensure(name)
            return name
        if "const" in schema:
            return self._kotlin_literal(schema["const"])
        if "enum" in schema:
            return self._enum_name(schema.get("title", "Enum"))
        # Check anyOf/oneOf for null — collapse nullable unions
        for key in ("anyOf", "oneOf"):
            if key in schema:
                variants = schema[key]
                has_null = any(v.get("type") == "null" or "null" in v.get("type", []) for v in variants)
                non_null = [v for v in variants if not (v.get("type") == "null" or "null" in v.get("type", []))]
                if has_null and len(non_null) == 1:
                    return self.kotlin_type(non_null[0], inline_depth=inline_depth) + "?"
                if has_null and len(non_null) > 1:
                    rendered = list(dict.fromkeys(self.kotlin_type(v, inline_depth=inline_depth) for v in non_null))
                    return " | ".join(rendered) + "?"
                # No null, render all
                rendered = list(dict.fromkeys(self.kotlin_type(v, inline_depth=inline_depth) for v in variants))
                return " | ".join(rendered)
        t = schema.get("type")
        if isinstance(t, list):
            if "null" in t:
                rest = [x for x in t if x != "null"]
                inner = self.kotlin_type({**schema, "type": rest[0]} if len(rest) == 1 else {"type": rest[0]}, inline_depth=inline_depth)
                return f"{inner}?"
            return self.kotlin_type({**schema, "type": t[0]}, inline_depth=inline_depth)
        if t == "string":
            return "String"
        if t in ("integer", "number"):
            return "Int" if t == "integer" else "Double"
        if t == "boolean":
            return "Boolean"
        if t == "null":
            return "Unit"
        if t == "array":
            ...
            if items is None:
                return "List<Unit>"
            inner = self.kotlin_type(items, inline_depth=inline_depth + 1)
            return f"List<{inner.replace('?', '')}>"
        if t == "object" or "properties" in schema or "additionalProperties" in schema:
            return self._kotlin_object(schema, inline_depth)
        if not schema or set(schema) <= {"title", "description", "default"}:
            return "Unit"
        raise ValueError(f"unsupported JSON-Schema construct: {json.dumps(schema)[:200]}")

    def _enum_name(self, title: str) -> str:
        return _pascal(title)

    def _kotlin_literal(self, value) -> str:
        if isinstance(value, str):
            return f'"{value}"'
        return json.dumps(value)

    def _kotlin_object(self, schema: dict, depth: int) -> str:
        """Return a type string for inline objects. For named objects, ensure() handles it."""
        props = schema.get("properties")
        if not props:
            return "Unit?"
        return "Unit?"  # Named objects are emitted via ensure(); inline objects use the ref path
    ...
    def ensure(self, name: str) -> None:
        ...
        if "enum" in schema:
            ...
            entries = []
            for v in enum_vals:
                entries.append(f"    @SerialName({self._kotlin_literal(v)})\n    {_pascal(str(v))}")
            body = f"@Serializable\nenum class {name} {{\n{chr(10).join(entries)}\n}}"
            self.emitted[name] = doc + body
        elif schema.get("properties"):
            ...
            fields = []
            for key, sub in props.items():
                kt = self.kotlin_type(sub, inline_depth=1)
                fields.append(f"    @SerialName(\"{key}\")\n    val {key}: {kt}")
            fields_str = "\n\n".join(fields)
            body = f"@Serializable\ndata class {name}(\n{fields_str}\n)"
            self.emitted[name] = doc + body
        else:
            self.emitted[name] = doc + f"@Serializable\ntypealias {name} = Unit?"

    def render(self) -> str:
        for name in list(self.emitted.keys()):
            self.ensure(name)
        out = [_KOTLIN_HEADER]
        for name in self.emitted:
            if name in ("ConnectorErrorReason",):
                continue
            out.append(self.emitted[name])
            out.append("")
        return "\n".join(out)
...
def render_kotlin() -> str:
    ...
    models = list(seen)
    defs, tops = _schema_for(models)
    r = KotlinRenderer(defs)
    name_of = {m: r.ref_name(t["$ref"]) for m, t in zip(models, tops)}
    for m in models:
        r.ensure(name_of[m])
    out = r.render()
    # Add the method/request/event maps as Kotlin constants
    out += _kotlin_constants()
    return _tidy_kotlin(out)


def _kotlin_constants() -> str:
    lines = []
    lines.append("// ── Method names ──")
    lines.append("object RpcMethodNames {")
    for m in sorted(METHODS.values(), key=lambda x: x.name):
        lines.append(f"    const val {_pascal(m.name)} = \"{m.name}\"")
    lines.append("}")
    lines.append("")
    lines.append("object ServerRequestNames {")
    for s in sorted(SERVER_REQUESTS.values(), key=lambda x: x.name):
        lines.append(f"    const val {_pascal(s.name)} = \"{s.name}\"")
    lines.append("}")
    lines.append("")
    lines.append("object GatewayEventNames {")
    for e in sorted(EVENTS.values(), key=lambda x: x.name):
        lines.append(f"    const val {_pascal(e.name)} = \"{e.name}\"")
    lines.append("}")
    return "\n".join(lines) + "\n"


def _tidy_kotlin(text: str) -> str:
...
def render_all() -> dict[Path, str]:
    results = {TS_OUT: _tidy(render_ts()), OPENRPC_OUT: render_openrpc()}
    try:
        results[KOTLIN_OUT] = _tidy_kotlin(render_kotlin())
    except Exception as e:
        print(f"warning: kotlin render failed: {e}", file=sys.stderr)
    return results
